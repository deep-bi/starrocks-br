...
def mock_unhealthy_cluster(mocker):
    ...
    return mocker.patch('starrocks_br.health.check_cluster_health', return_value=(False, "Cluster is unhealthy"))
...
@pytest.mark.parametrize("command,baseline_flag", [
    ("backup_incremental", ["--baseline-backup", "test_baseline"]),
    ("backup_incremental", []),
    ("backup_full", []),
])
def test_backup_handles_snapshot_exists_error(
    config_file, mock_db, mock_initialized_schema, mock_healthy_cluster,
    mock_repo_exists, setup_password_env, mocker, command, baseline_flag
):
    ...
    if command == "backup_incremental":
        ...
        mocker.patch('starrocks_br.planner.build_incremental_backup_command',
        ...
        mocker.patch('starrocks_br.planner.build_full_backup_command',
                    return_value='BACKUP DATABASE test_db SNAPSHOT test_backup_20251020 TO test_repo')
...
@pytest.mark.parametrize("command,args_factory", [
...
def test_commands_exit_if_schema_is_auto_created(
    config_file, mock_db, mock_uninitialized_schema, setup_password_env,
    mocker, command, args_factory
):
...
@pytest.mark.parametrize("command,exception,expected_msg", [
...
def test_commands_handle_exceptions(
    ...
    if command == "cli":
        # For restore command
        result = runner.invoke(cli.cli, ['restore', '--config', config_file, '--target-label', 'test'])
    elif command == "init":
...
@pytest.mark.parametrize("scenario,mock_setup,expected_msg", [
    (
        "LOST state in incremental backup",
        {
            'command': 'backup_incremental',
            'result': {'success': False, 'final_status': {'state': 'LOST'}, 'error_message': 'Tracking lost'},
        },
...
def test_backup_logic_failures(
    config_file, mock_db, mock_initialized_schema, mock_healthy_cluster,
    mock_repo_exists, setup_password_env, mocker, scenario, mock_setup, expected_msg
):
    ...
    if command == 'backup_incremental':
        latest_backup = mock_setup.get('latest_backup', {
            'label': 'test_db_20251015_full',
            'backup_type': 'full',
            'finished_at': '2025-10-15 10:00:00'
        })
        ...
        partitions = mock_setup.get('partitions', [
            {"database": "test_db", "table": "fact_table", "partition_name": "p20251016"}
        ])
        ...
        backup_command = mock_setup.get('backup_command', 'BACKUP DATABASE test_db SNAPSHOT test_backup TO test_repo')
...
@pytest.mark.parametrize("scenario,mock_behavior,expected_msg", [
    (
...
def test_restore_logic_failures(
    config_file, mock_db, mock_initialized_schema, mock_healthy_cluster,
    mock_repo_exists, setup_password_env, mocker, scenario, mock_behavior, expected_msg
):
    ...
    if 'find_restore_pair' in mock_behavior:
        mocker.patch('starrocks_br.restore.find_restore_pair', side_effect=mock_behavior['find_restore_pair'])
    else:
    ...
    if 'get_tables_from_backup' in mock_behavior:
        if isinstance(mock_behavior['get_tables_from_backup'], list):
            mocker.patch('starrocks_br.restore.get_tables_from_backup',
...
def test_backup_incremental_success(
    config_file, mock_db, mock_initialized_schema, mock_healthy_cluster,
    mock_repo_exists, setup_password_env, mocker
):
...
def test_backup_incremental_with_specific_baseline(
    ...
    result = runner.invoke(cli.backup_incremental, [
        '--config', config_file,
        '--baseline-backup', 'test_db_20251010_full',
...
def test_backup_full_success(
    ...
    mocker.patch('starrocks_br.planner.find_tables_by_group', return_value=[
        {'database': 'test_db', 'table': 'dim_customers'}
    ])
    mocker.patch('starrocks_br.planner.get_all_partitions_for_tables', return_value=[])
...
def test_restore_with_group_filter(
    ...
    mocker.patch('starrocks_br.restore.find_restore_pair', return_value=['test_backup'])
    get_tables_mock = mocker.patch('starrocks_br.restore.get_tables_from_backup',
                                  return_value=['test_db.fact_table'])
    mocker.patch('starrocks_br.restore.execute_restore_flow', return_value={
        'success': True,
        'message': 'Restore completed successfully. Restored 1 tables.'
    })
...
@pytest.mark.parametrize("table_value,expected_msg", [
    ("test_db.fact_table", "Table name must not include database prefix"),
    ("   ", "Table name cannot be empty"),
])
def test_restore_table_validation(config_file, mock_db, mock_initialized_schema,
                                 setup_password_env, table_value, expected_msg):
...
def test_backup_incremental_unhealthy_cluster(
    config_file, mock_db, mock_initialized_schema, mock_unhealthy_cluster,
    setup_password_env
):
...
@pytest.mark.parametrize("command,setup_mocks", [
    ("backup_incremental", {
        'find_latest_full_backup': {'label': 'test_db_20251015_full', 'backup_type': 'full', 'finished_at': '2025-10-15 10:00:00'},
        'find_recent_partitions': [{"database": "test_db", "table": "fact_table", "partition_name": "p20251016"}],
...
def test_backup_fails_with_non_lost_state(
    config_file, mock_db, mock_initialized_schema, mock_healthy_cluster,
    mock_repo_exists, setup_password_env, mocker, command, setup_mocks
):
...
@pytest.mark.parametrize("filter_type,filter_value,expected_line", [
    ("group", "nonexistent_group", "No tables found in backup 'test_backup' for group 'nonexistent_group'"),
    ("table", "nonexistent_table", "No tables found in backup 'test_backup' for table 'nonexistent_table'"),
])
def test_restore_no_tables_found_with_filters(
    config_file, mock_db, mock_initialized_schema, mock_healthy_cluster,
    mock_repo_exists, setup_password_env, mocker, filter_type, filter_value, expected_line
):
