...
def test_should_create_database_connection_object():
    conn = db.StarRocksDB(
        host='127.0.0.1',
        port=9030,
        user='root',
...
def test_should_execute_sql_statement(mocker):
    conn = db.StarRocksDB('localhost', 9030, 'root', '', 'test_db')
...
def test_should_query_and_return_results(mocker):
    conn = db.StarRocksDB('localhost', 9030, 'root', '', 'test_db')
...
def test_should_support_context_manager(mocker):
    mock_connection = mocker.Mock()
    mocker.patch('mysql.connector.connect', return_value=mock_connection)
